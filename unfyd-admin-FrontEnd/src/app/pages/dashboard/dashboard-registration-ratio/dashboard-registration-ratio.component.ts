...
@Component({
  selector: 'app-dashboard-registration-ratio',
  templateUrl: './dashboard-registration-ratio.component.html',
  styleUrls: ['./dashboard-registration-ratio.component.scss']
})
export class DashboardRegistrationRatioComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getInformation("usp_unfyd_registration_ratio_dashboard", "Day")
    ...
    if (direction == 'next') {
      if (this.selectedGrievance == 'Day') {
        ...
        if (this.hawkerAppDate.getMonth() == 11) {
          ...
          this.getInformation("usp_unfyd_registration_ratio_dashboard", "Month")
        ...
        this.getInformation("usp_unfyd_registration_ratio_dashboard", "Year")
  ...
  getTomorrowDate() {
    ...
    this.hawkerAppDate 
    = new Date(this.hawkerAppDate.getTime() + 24 * 60 * 60 * 1000)
  }
  ...
  getInformation(spname, flag) {
    ...
    this.api.post(this.endPoint, obj).subscribe(res => {
      ...
      if (res.code == 200) {
        ...
        this.keys = [];
        this.values = [];
        this.keys = Object.keys(res.results.data[0])
        this.values = Object.values(res.results.data[0])
        if (this.selectedGrievance == 'Day') {
  ...
  getGraphData() {
    ...
    {
      xAxis: {
        data: ['Applicants - Registrations'],
        axisTick: { show: false },
        axisLabel: {
          formatter: "Applicants - Registrations"
        }
      },
      tooltip:{
        trigger:'none',
        formatter: `Applicants:{c}\n Registrations{{this.value[1]}}`
      },
      label: {
        show: true,
        position: 'top'
      ...
      grid: { containLabel:true ,top:20,left:-20, bottom:0,right:0,show:false },
      series: [
        {
          type: 'bar',
          barWidth: 60,
          barGap: '-50%',
          data: [{ value: this.values[0], itemStyle: { color: '#ECF2FB', borderColor: '#ECF2FB' } }],
          itemStyle: {
        ...
        },
        {
          ...
          barWidth: 60,
          z: 10,
          data: [{ value: this.values[1], itemStyle: { color: '#967BDC', borderColor: '#967BDC' } }],
          itemStyle: {
            borderRadius: [7, 7, 7, 7],
            borderColor: '#967BDC'
          }
