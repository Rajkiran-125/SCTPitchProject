...
@Component({
  selector: 'app-dashboard-top-performer-by-location',
  templateUrl: './dashboard-top-performer-by-location.component.html',
  styleUrls: ['./dashboard-top-performer-by-location.component.scss']
})
export class DashboardTopPerformerByLocationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getInformation("usp_unfyd_top_performer_location_dashboard", "Day")
    ...
    if (direction == 'next') {
      if (this.selectedGrievance == 'Day') {
        ...
        if (this.hawkerAppDate.getMonth() == 11) {
          ...
          this.getInformation("usp_unfyd_top_performer_location_dashboard", "Month")
        ...
        this.getInformation("usp_unfyd_top_performer_location_dashboard", "Year")
    ...
    this.api.post(this.endPoint, obj).subscribe(res => {
      ...
      if (res.code == 200) {
        ...
        let a = 5 - this.topPerformers.length;
                if(a<=5){
                  for (var i = 0; i < a; i++) {
                    this.topPerformers.push({'Location':'-','Count':'-'})
                }
                }
        if (this.selectedGrievance == 'Day') {
