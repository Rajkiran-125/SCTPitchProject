...
@Component({
  selector: 'app-email-routing',
  templateUrl: './email-routing.component.html',
  styleUrls: ['./email-routing.component.scss']
})
export class EmailRoutingComponent implements OnInit {
  key: any;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      Webhook_URL: ['', [Validators.required, Validators.pattern(regex.urlregexwithhttp)]],
      Queue_Name: ['', [Validators.required]],
      Queue_API: ['', [Validators.required]],
      Delete_Email_Post_Processing: [false, Validators.nullValidator]

    },{validator: [checknull('Webhook_URL'),checknull('Queue_Name'),checknull('Queue_API')]})


    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/email-configuration/email-settings', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        ...
        this.api.post('index', obj).subscribe(res => {
          ...
          if (res.code == 200) {
            ...
            for (var i = 0; i < tmpt.length; ++i) {
              ...
              if(tmpt[i].ConfigKey == "Delete_Email_Post_Processing" )
              {
    ...
    this.dataTemp.forEach((element, index) => {
      keys.forEach((object, j) => {
        if (element.Key == object) {
          ...
          if(element.Key  == "Webhook_URL" || element.Key  =="Queue_Name" || element.Key  =="Queue_API")
          {
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if (event == 'saveAddNew') {
              this.router.navigate(['masters/channel-configuration/email-configuration/account-credentials', this.activatedRoute.snapshot.paramMap.get('id'), 'add']);
            } else if (event == 'save') {
              this.router.navigate(['masters/channel-configuration/email-configuration/view', this.activatedRoute.snapshot.paramMap.get('id')]);
            }
          }
          cntr++;
        } else {
