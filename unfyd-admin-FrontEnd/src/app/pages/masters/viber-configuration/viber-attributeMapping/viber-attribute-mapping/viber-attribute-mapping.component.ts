...
@Component({
  selector: 'app-viber-attribute-mapping',
  templateUrl: './viber-attribute-mapping.component.html',
  styleUrls: ['./viber-attribute-mapping.component.scss']
})
export class ViberAttributeMappingComponent implements OnInit {
  ...
  viberdata: { data: { spname: string; parameters: { flag: string; processid: any; productid: number; CHANNELID: number; }; }; };
  viberchannel: any;
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/viber-configuration/viber-detail', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
  ...
  getviberlist() {
    ...
    this.api.post('index', this.viberdata).subscribe((res) => {
      if (res.code == 200) {
        this.loader = false;
        let a: any = [];
        var temp = []
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/viber-configuration/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
