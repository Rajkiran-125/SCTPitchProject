...
@Component({
  selector: 'app-viber-details',
  templateUrl: './viber-details.component.html',
  styleUrls: ['./viber-details.component.scss']
})
export class ViberDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    { validator: [checknull('ChannelSourceName'),checknull1('ChannelSourceName'),checknull('VersionNumber'),checknull1('VersionNumber'),checknull('SendMessageURL'),checknull1('SendMessageURL'),,checknull('ScenarioKey'),checknull1('ScenarioKey')] },);
  }
  ...
  SaveViber(event){
    // this.router.navigate(['masters/channel-configuration/viber-configuration/viber-authType/40/add']);
        ...
        if (res.code == 200) {
          if(res.results.data[0].result == 'success')
          {
            let ChSrcId = res.results.data[0].Id
                ...
                let counter = 0;
                this.viberuniquedata = this.channel?.ChannelName.toUpperCase() + dd + mm + yyyy + time;
                for (let i = 0; i < temp.length; i++) {
                  if(temp[i] == "ChannelSourceName" || temp[i] == "VersionNumber" || temp[i] == "SendMessageURL"  || temp[i] == "ScenarioKey" )
                  {
                  ...
                  this.api.post('index', obj).subscribe((res: any) => {
                    if (res.code == 200) {
                      if (temp.length - 1 == counter) {
                        ...
                        if(event == 'next'){
                          this.router.navigate(['masters/channel-configuration/viber-configuration/viber-authType', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);

                        }
                        else if(event == 'save') {
                          this.router.navigate(['masters/channel-configuration/viber-configuration/viber-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);
                          this.isUpdate = 'true'
                        }
      ...
      this.dataTemp.forEach((element, index) => {
        keys.forEach((object, j) => {
          if (element.Key == object) {
            if(element.Key  == "ChannelSourceName" || element.Key  == "VersionNumber" || element.Key  == "SendMessageURL" || element.Key  == "ScenarioKey")
            {
          ...
          if (res.code == 200) {
            ...
            if (counter == 0) {
              ...
              if(event == 'next'){
                ...
                this.router.navigate(['masters/channel-configuration/viber-configuration/viber-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
              }            }
          } else {
            this.loader = false;
          }
