...
@Component({
  selector: 'app-viber-media-details',
  templateUrl: './viber-media-details.component.html',
  styleUrls: ['./viber-media-details.component.scss']
})
export class ViberMediaDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      MediaDownloadAPI: ['', [Validators.required]],
  ...
  SaveViber(event){
    // this.router.navigate(['masters/channel-configuration/viber-configuration/viber-database/40/add']);
    ...
    this.dataTemp.forEach((element, index) => {
      keys.forEach((object, j) => {
        if (element.Key == object) {
          ...
          if(element.Key  == "MediaDownloadAPI" || element.Key  == "PageAccessToken" || element.Key  == "StorageLocation")
          {
