...
@Component({
  selector: 'app-viber-data-management-api',
  templateUrl: './viber-data-management-api.component.html',
  styleUrls: ['./viber-data-management-api.component.scss']
})
export class ViberDataManagementAPIComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      SaveIncomingData: ['', [Validators.required]],
      SaveOutgoingData: ['', [Validators.required]],
      CreateCustomerAgentSession: ['', [Validators.required]],
      DeleteCustomerAgentSession: ['', [Validators.required]],
      ValidateCustomerAgentSession: ['', [Validators.required]],
    },
  ...
  SaveViber(event){
    // this.router.navigate(['masters/channel-configuration/viber-configuration/viber-MediaDetails/40/add']);
    ...
    this.dataTemp.forEach((element, index) => {
      keys.forEach((object, j) => {
        if (element.Key == object) {
          ...
          if(element.Key  == "SaveIncomingData" || element.Key  == "SaveOutgoingData" || element.Key  == "CreateCustomerAgentSession" || element.Key  == "DeleteCustomerAgentSession"|| element.Key  == "ValidateCustomerAgentSession")
          {
        ...
        if (res.code == 200) {
          ...
          if (updateData.length == cntr) {
            ...
            if (event == 'save') {
              ...
              this.router.navigate(['masters/channel-configuration/viber-configuration/viber-MediaDetails', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            }
          }
          cntr++;
        } else {
