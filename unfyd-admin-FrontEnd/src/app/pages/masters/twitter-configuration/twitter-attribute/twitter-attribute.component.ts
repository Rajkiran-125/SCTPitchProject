...
import { masters, regex, countryCode, twitterConfigurationsteps } from 'src/app/global/json-data';

@Component({
  selector: 'app-twitter-attribute',
  templateUrl: './twitter-attribute.component.html',
  styleUrls: ['./twitter-attribute.component.scss']
})
export class TwitterAttributeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/twitter-configuration/twitter-database', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/twitter-configuration/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
