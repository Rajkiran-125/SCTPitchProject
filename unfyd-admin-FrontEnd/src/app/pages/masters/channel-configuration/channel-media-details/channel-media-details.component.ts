...
@Component({
  selector: 'app-channel-media-details',
  templateUrl: './channel-media-details.component.html',
  styleUrls: ['./channel-media-details.component.scss']
})
export class ChannelMediaDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      Media_Download_API: ['', [Validators.required, Validators.pattern(regex.urlregexwithhttp)]],
      Authentication_API: ['', [Validators.required, Validators.pattern(regex.alphabet)]],
      Storage_Location: ['', [Validators.required, Validators.pattern(regex.alphabet)]],
      Enable_Auto_Download: [false, Validators.nullValidator]

    },
    {validator:[checknull('Media_Download_API'),checknull('Authentication_API'),checknull('Storage_Location')]}
      ...
      this.api.post('index', obj).subscribe(res => {
        ...
        if (res.code == 200) {
          ...
          for (var i = 0; i < tmpt.length; ++i) {
            ...
            if(tmpt[i].ConfigKey == 'Enable_Auto_Download'){


              let val ;
              if(tmpt[i].ConfigValue == "true")
              {
                val = true;
              }
              if(tmpt[i].ConfigValue == "false")
              {
                val = false;
              }

              this.form.patchValue({ [tmpt[i].ConfigKey]: val })

              this.form.get('Enable_Auto_Download').updateValueAndValidity()
    ...
    this.dataTemp.forEach((element, index) => {
      keys.forEach((object, j) => {
        if (element.Key == object) {
          ...
          if(element.Key  !== "Enable_Auto_Download" )
            {
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') {this.router.navigate(['/masters/channel-configuration/channel-media-details', this.path, 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);}
             else if(event == 'next') this.router.navigate(['masters/channel-configuration/channel-database', this.path, 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
  ...
  getWhatsapplist() {
    ...
    this.common.hubControlEvent('ChannelConfiguration','click','','',JSON.stringify(this.whtsdata),'savewhatsapp');
