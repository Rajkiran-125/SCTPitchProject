...
@Component({
  selector: 'app-hsm-api',
  templateUrl: './hsm-api.component.html',
  styleUrls: ['./hsm-api.component.scss']
})
export class HsmApiComponent implements OnInit {
  channelname: any = '';
  key: any;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      Send_HSM: ['', [Validators.required]],
      storeHSM: ['', [Validators.required]],
      // authType: ['basic', [Validators.required]],
      // // source: ['', [Validators.required]],
      // sendmessagelength: ['', [Validators.required,Validators.pattern(regex.numeric)]],
      // sendmessageurl: ['', [Validators.required]],

      // quickreplyenable: [false, [Validators.nullValidator]],
      // quickreplybuttonlength: ['', [Validators.nullValidator]],
      // quickreplytitlelength: ['', [Validators.nullValidator]],
      // quickreplyurl: ['', [Validators.nullValidator]],

      // listoptionsenable: [false, [Validators.nullValidator]],
      // listoptionsurl: ['', [Validators.nullValidator]],
      // listoptionslength: ['', [Validators.nullValidator]],


      // authtoken: ['', [Validators.nullValidator]],

      // tokenurl: ['', [Validators.nullValidator]],
      // tokenusername: ['', [Validators.nullValidator]],
      // tokenpassword: ['', [Validators.nullValidator]],
      // tokenValidityDuration: ['', [Validators.nullValidator]],
      // tokentransactionapi: ['', [Validators.nullValidator]],
    },
    {validator:[checknull('Send_HSM'),checknull('storeHSM')]},
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') { this.router.navigate(['masters/channel-configuration/channel-configuration-edit/', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);}
            else if(event == 'next')  this.router.navigate(['/masters/channel-configuration/channel-details', this.path, 'add'])
           }
          cntr++;
        } else {
          this.loader = false;
