...
@Component({
  selector: 'app-facebook-messenger-database',
  templateUrl: './facebook-messenger-database.component.html',
  styleUrls: ['./facebook-messenger-database.component.scss']
})
export class FacebookMessengerDatabaseComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    {validator:[checknull('DBName'),checknull1('DBName'),checknull('UserName'),checknull1('UserName'),checknull('DB_Password'),checknull1('DB_Password'),checknull('HostName'),checknull1('HostName'),checknull('PortNo'),checknull1('PortNo'),checknull('IP'),checknull1('IP'),checknull1('Password')]}
    )
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {

      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/facebook-messenger/facebook-messenger-media', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/facebook-messenger/facebook-messenger-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/facebook-messenger/facebook-messenger-attribute', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
