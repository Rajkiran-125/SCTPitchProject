...
@Component({
  selector: 'app-facebook-messenger-media',
  templateUrl: './facebook-messenger-media.component.html',
  styleUrls: ['./facebook-messenger-media.component.scss']
})
export class FacebookMessengerMediaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    {validator:[checknull('MediaDownloadAPI'),checknull1('MediaDownloadAPI'),checknull('Storage_Location'),checknull1('Storage_Location')]}
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/facebook-messenger/facebook-messenger-api', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/facebook-messenger/facebook-messenger-media', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/facebook-messenger/facebook-messenger-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
