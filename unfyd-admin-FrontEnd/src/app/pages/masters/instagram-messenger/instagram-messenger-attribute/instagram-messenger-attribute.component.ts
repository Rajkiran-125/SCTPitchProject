...
import { masters, regex, countryCode, intagramMessengerSteps } from 'src/app/global/json-data';

@Component({
  selector: 'app-instagram-messenger-attribute',
  templateUrl: './instagram-messenger-attribute.component.html',
  styleUrls: ['./instagram-messenger-attribute.component.scss']
})
export class InstagramMessengerAttributeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/instagram-post/instagram-post-database', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/instagram-post/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
