...
@Component({
  selector: 'app-instagram-messenger-media',
  templateUrl: './instagram-messenger-media.component.html',
  styleUrls: ['./instagram-messenger-media.component.scss']
})
export class InstagramMessengerMediaComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    {validator:[checknull('MediaDownloadAPI'),checknull1('MediaDownloadAPI'),checknull('StorageLocation'),checknull1('StorageLocation'),checknull1('PageAccessToken'),checknull('PageAccessToken')]}
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/instagram-post/instagram-post-api', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/instagram-post/instagram-post-media', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/instagram-post/instagram-post-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
