...
export class VoiceConfigurationComponent implements OnInit {
  form: FormGroup;
  ...
  this.api.post('index', this.emaildata).subscribe((res) => {
    if (res.code == 200) {
      ...
      this.voicechannel = res.results.data;
      this.voicechannel.forEach(element => {
      ...
      this.stringJson = JSON.parse(JSON.stringify(this.voicechannel));
      this.channelData = a;
    }
  });
  }
