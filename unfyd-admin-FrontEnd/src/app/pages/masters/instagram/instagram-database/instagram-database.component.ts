...
@Component({
  selector: 'app-instagram-database',
  templateUrl: './instagram-database.component.html',
  styleUrls: ['./instagram-database.component.scss']
})
export class InstagramDatabaseComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      UserName: ['', [Validators.required,Validators.pattern(regex.alphanumericwithspecialcharacter)]],
      Password: ['', [Validators.required,Validators.pattern(regex.alphanumericwithspecialcharacter)]],
      HostName: ['', [Validators.required,Validators.pattern(regex.alphanumericwithspecialcharacter)]],
      PortNo: ['', [Validators.required]],
      IP: ['', [Validators.required, Validators.pattern(regex.ipaddress)]]
    },
    {validator:[checknull('DBName'),checknull1('DBName'),checknull('UserName'),checknull1('UserName'),checknull('Password'),checknull1('Password'),checknull('HostName'),checknull1('HostName'),checknull('PortNo'),checknull1('PortNo'),checknull('IP'),checknull1('IP')]}
    )
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {

      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/instagram/instagram-media', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
        this.editInstagramDm('edit', this.uniqueId)
      }
    }
    this.getChannel();
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/instagram/instagram-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/instagram/instagram-attribute', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
