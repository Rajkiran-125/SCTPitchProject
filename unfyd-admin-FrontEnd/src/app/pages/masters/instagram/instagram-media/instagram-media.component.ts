...
@Component({
  ...
  styleUrls: ['./instagram-media.component.scss']
})
export class InstagramMediaComponent implements OnInit {
    ...
    ngOnInit(): void {
      ...
      {validator:[checknull('MediaDownloadAPI'),checknull1('MediaDownloadAPI'),checknull1('StorageLocation'),checknull('StorageLocation'),checknull1('PageAccessToken'),checknull('PageAccessToken')]}
      ...
      if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
        ...
        this.loader = false;
        this.router.navigate(['/masters/channel-configuration/instagram/instagram-api', this.path, 'add'])
      }
      else {
        if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
          this.isUpdate = 'true';
        ...
        this.api.post('index', obj).subscribe((res: any) => {
          if (res.code == 200) {
            ...
            if (updateData.length - 1 == cntr) {
              ...
              if(event == 'save') this.router.navigate(['masters/channel-configuration/instagram/instagram-media', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
              else if(event == 'next')this.router.navigate(['masters/channel-configuration/instagram/instagram-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
