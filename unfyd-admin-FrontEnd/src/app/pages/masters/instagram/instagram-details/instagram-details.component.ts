...
import { masters, regex, countryCode, intagramsteps, checknull, checknull1 } from 'src/app/global/json-data';
...
export class InstagramDetailsComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  constructor(public common: CommonService,
    ...
    private el: ElementRef,) {   Object.assign(this, { masters, regex, countryCode, intagramsteps }); }
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/instagram/instagram-details', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
    ...
    this.form = this.formBuilder.group({
      ...
      ScenarioKey: ['', Validators.required],
      // auth
      AccessToken:[''],
      VerifyTokenName:[],
    ...
    { validator: [checknull('ChannelSourceName'),checknull1('ChannelSourceName'),checknull('SendMessageUrl'),checknull1('SendMessageUrl'), checknull('VersionNumber'),checknull1('VersionNumber'), checknull('ScenarioKey'),checknull1('ScenarioKey'),checknull('PageID'),checknull1('PageID') ] },
    )
  ...
  back(): void{
    this.router.navigate(['masters/channel-configuration/instagram/view/'+this.path]);
  }
  hideChange() {
    this.hide = !this.hide
  }
          ...
          if(res.results.data[0].result == 'success')          {
              let ChSrcId = res.results.data[0].Id
                ...
                for (let i = 0; i < temp.length; i++) {
                  if(temp[i] == "ChannelSourceName" || temp[i] =="VersionNumber"  || temp[i] =="SendMessageUrl" || temp[i] =="PageID" || temp[i] =="ScenarioKey")
                  {
                  ...
                  this.api.post('index', obj).subscribe((res: any) => {
                    if (res.code == 200) {
                      if (temp.length - 1 == counter) {
                        this.loader = false;
                        if(event == 'save') {this.router.navigate(['masters/channel-configuration/instagram/instagram-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);  this.isUpdate = 'true';}
                        else if(event == 'next')this.router.navigate(['masters/channel-configuration/instagram/instagram-auth', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);
      ...
      this.dataTemp.forEach((element, index) => {
        keys.forEach((object, j) => {
          if (element.Key == object) {
            if(element.Key  == "ChannelSourceName" || element.Key  =="VersionNumber"|| element.Key  =="SendMessageUrl" || element.Key  =="PageID" || element.Key  =="ScenarioKey")
            {
        ...
        this.api.post('index', obj).subscribe((res: any) => {
          if (res.code == 200) {
            ...
            if (counter == 0) {
              ...
              if(event == 'save') this.router.navigate(['masters/channel-configuration/instagram/instagram-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
              else if(event == 'next')this.router.navigate(['masters/channel-configuration/instagram/instagram-auth', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
