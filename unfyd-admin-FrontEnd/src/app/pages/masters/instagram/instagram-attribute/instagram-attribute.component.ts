...
import { masters, regex, countryCode, intagramsteps } from 'src/app/global/json-data';

@Component({
  selector: 'app-instagram-attribute',
  templateUrl: './instagram-attribute.component.html',
  styleUrls: ['./instagram-attribute.component.scss']
})
export class InstagramAttributeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/instagram/instagram-database', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/instagram/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
