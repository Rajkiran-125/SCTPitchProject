...
export class OtherDetailsComponent implements OnInit {
  ...
  getSnapShot() {
    ...
    if (this.path !== null) {
      ...
      this.requestObj = {
        data: {
          spname: "usp_unfyd_trainer_other",
          parameters: {
            flag: 'EDIT',
            contactid: this.path
          }
  ...
  submit(btnType): void {
    ...
    this.api.post('index', this.requestObj).subscribe(res => {
      if (res.code == 200) {
        this.loader = false;
        if (btnType == 'next') {
          this.router.navigate(['masters/trainers/kyc', this.commonObj.contactid]);
        } else if (btnType == 'save') {
          this.router.navigate(['masters/trainers/other-details', this.commonObj.contactid]);
        }
