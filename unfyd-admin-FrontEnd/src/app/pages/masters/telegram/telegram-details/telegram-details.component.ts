...
@Component({
  selector: 'app-telegram-details',
  templateUrl: './telegram-details.component.html',
  styleUrls: ['./telegram-details.component.scss']
})
export class TelegramDetailsComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/telegram/telegram-details', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
    ...
    this.form = this.formBuilder.group({
      ...
      WebhookID: ['', [Validators.required]],
      QuickReply: ['', [Validators.required,Validators.pattern(regex.numeric)]],
      QuickReplyTitle: ['', [Validators.required,Validators.pattern(regex.numeric)]],
      // auth
      PageAccessToken:[''],
      VerifyTokenName:[],
    ...
    { validator: [checknull('ChannelSourceName'),checknull1('ChannelSourceName'),checknull('SendMessageUrl'),checknull1('SendMessageUrl'), checknull('VersionNumber'),checknull1('VersionNumber'),checknull1('WebhookID'), checknull('WebhookID'), ] },
    )
        ...
        if (res.code == 200) {
          if(res.results.data[0].result == 'success')          {
              let ChSrcId = res.results.data[0].Id
                ...
                let counter = 0;
                this.telegramuniquedata = this.channel?.ChannelName.toUpperCase() + dd + mm + yyyy + time;
                for (let i = 0; i < temp.length; i++) {
                  if(temp[i] == "ChannelSourceName" || temp[i] =="VersionNumber"  || temp[i] =="SendMessageUrl" || temp[i] =="WebhookID" )
                  {
                  ...
                  let obj = {
                    data: {
                      ...
                      parameters: {
                        ...
                        processid: this.userDetails.Processid,
                        productid: 1,
                        uniqueid: this.telegramuniquedata,
                  ...
                  this.api.post('index', obj).subscribe((res: any) => {
                    if (res.code == 200) {
                      if (temp.length - 1 == counter) {
                        this.loader = false;
                        if(event == 'save') {this.router.navigate(['masters/channel-configuration/telegram/telegram-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);  this.isUpdate = 'true';}
                        else if(event == 'next')this.router.navigate(['masters/channel-configuration/telegram/telegram-auth', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);
      ...
      this.dataTemp.forEach((element, index) => {
        keys.forEach((object, j) => {
          if (element.Key == object) {
            if(element.Key  == "ChannelSourceName" || element.Key  =="VersionNumber"|| element.Key  =="SendMessageUrl" || element.Key  =="WebhookID" )
            {
        ...
        this.api.post('index', obj).subscribe((res: any) => {
          if (res.code == 200) {
            ...
            if (counter == 0) {
              ...
              if(event == 'save') this.router.navigate(['masters/channel-configuration/telegram/telegram-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
              else if(event == 'next')this.router.navigate(['masters/channel-configuration/telegram/telegram-auth', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
