...
import { masters, regex, countryCode, checknull, telegramSteps, checknull1 } from 'src/app/global/json-data';

@Component({
  selector: 'app-telegram-database',
  templateUrl: './telegram-database.component.html',
  styleUrls: ['./telegram-database.component.scss']
})
export class TelegramDatabaseComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    {validator:[checknull('DBName'),checknull1('DBName'),checknull('DB_Username'),checknull1('DB_Username'),checknull('DB_Password'),checknull1('DB_Password'),checknull('HostName'),checknull1('HostName'),checknull('PortNo'),checknull1('PortNo'),checknull('IP'),checknull1('IP'),checknull1('UserName'),checknull1('Password')]}
    )
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {

      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/telegram/telegram-media', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/telegram/telegram-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/telegram/telegram-attribute', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
