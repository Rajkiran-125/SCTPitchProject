...
import { masters, regex, countryCode, telegramSteps } from 'src/app/global/json-data';

@Component({
  selector: 'app-telegram-attribute',
  templateUrl: './telegram-attribute.component.html',
  styleUrls: ['./telegram-attribute.component.scss']
})
export class TelegramAttributeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/telegram/telegram-database', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/telegram/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
