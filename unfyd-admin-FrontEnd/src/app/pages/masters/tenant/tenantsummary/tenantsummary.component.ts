...
@Component({
  selector: 'app-tenantsummary',
  templateUrl: './tenantsummary.component.html',
  styleUrls: ['./tenantsummary.component.scss']
})
...
/////////////////////////////////////////////////////
    ...
    async Channelloopfunc()
    {
      this.channelcount = this.editObj1.length;
      // this.editObj1.forEach(async element => {  
    console.log('channeltuype',this.channelType);
    
        for(let i=0; i<this.editObj1.length; i++){
                this.loader = true;

      //                 let chidarr = element.ChannelId;
      // let chproidarr = element.UnfydProductId;
      // let chlicensecut = element.LicenseCount;
                let chidarr = this.editObj1[i].ChannelId;
                let chproidarr = this.editObj1[i].UnfydProductId;
                let chlicensecut = this.editObj1[i].LicenseCount;
            

                const index = await this.channelType.findIndex(object => {     
                  return object.ChannelId == chidarr;
                });
              this.a[chproidarr]?.channel['channelid'].push(this.channelType[index]); 

                // let index =  this.channelType.filter(object => {
                //  return object.ChannelId == chidarr
                // });
            
                
                // this.a[chproidarr]?.channel['channelid'].push(index[0]); 
...
this.api.post('index', this.requestObj).subscribe((res: any) => {
  if (res.code == 200) {
    if(this.loginImg == undefined)
    {
      ...
      this.common.snackbar('File Upload Sucess');
    }
...
fileType: boolean = false;
 ...
 this.findextension = type;
  ...
  && type !== 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'  && type !== 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
  && type !== 'text/plain') {
    event = null;
    this.disabled = true
    this.imgPreview = null
    this.fileType = true
    this.common.snackbar("upload Image, Pdf, docx, xlsx file");
  } 
