...
import { masters, regex, countryCode, facebookConfigurationsteps } from 'src/app/global/json-data';

@Component({
  selector: 'app-facebook-attribute',
  templateUrl: './facebook-attribute.component.html',
  styleUrls: ['./facebook-attribute.component.scss']
})
export class FacebookAttributeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/facebook-configuration/facebook-database', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/facebook-configuration/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
