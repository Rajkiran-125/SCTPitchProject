...
@Component({
  ...
  styleUrls: ['./solution-manager.component.scss'],
  providers: [
...
export class SolutionManagerComponent implements OnInit {
  ...
  UpdateNotificationsToggleValue:  boolean = false;
  UpdateLogToggleValue:  boolean = false;
  constructor(
  ...
  getbroadcast(event) {
    ...
    //   })
      ...
      this.api.pm2Api('Get', obj).subscribe(res => {
        if (res.code == 200) {
          this.loader = false;
          this.tabValue = res.results;
  ...
  ViewData(data){
    this.RouteData = data
    this.router.navigate(['/masters/' + this.type + '/update/', data.Health.Id]);
  ...
  GetPm2RouteData(){
    ...
    this.api.pm2Api('Get', obj).subscribe(res => {
      if (res.code == 200) {
        ...
        this.RouteData = data.filter(item => {
              if (parseInt(item.Health.Id) == parseInt(this.SolutionId))
              {
                return true;
              }
          return false;
          ...
          console.log('this.RouteData',this.RouteData);
          console.log('this.RouteData',this.RouteData[0].Health);
          this.loader = false;
      }
    },
      (error) => {
  ...
  RestartPm2Api(){
    ...
    this.api.pm2Api('Restart', obj).subscribe(res => {
      if (res.code == 200) {
        this.loader = false;
        this.common.snackbar("ApiRestarted");
          this.GetPm2RouteDataFunc()
      }
    },
      (error) => {
        this.common.snackbar("General Error");
  ...
  StopPm2Api(){
    ...
    this.api.pm2Api('Stop', obj).subscribe(res => {
      if (res.code == 200) {
        this.loader = false;
        this.common.snackbar("ApiStopped");
          this.GetPm2RouteDataFunc()
      }
    },
      (error) => {
  ...
  submit(event): void {
    ...
    let startdate=starts.getDate()
    console.log('startdate',startdate);
    
    let Ends = new Date(this.form.value.EndDate)
    let enddate=Ends.getDate()
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      this.SolutionId = params.Id})
      // console.log(' this.SolutionId = params.Id', this.SolutionId)
...
sortUsers(by: string, order: string): void {
  ...
  flip2(action,event)
  {
    console.log('2action,event2',action,event);
    
  }

  UpdateLogToggle(val){
    console.log('UpdateLogToggle',val);
   this.UpdateLogToggleValue=val
   console.log('UpdateLogToggleValue',this.UpdateLogToggleValue);
  }

  UpdateNotificationsToggle(val){
    console.log('UpdateNotificationsToggle',val);
    this.UpdateNotificationsToggleValue=val
    console.log('UpdateNotificationsToggleValue',this.UpdateNotificationsToggleValue);
  }
