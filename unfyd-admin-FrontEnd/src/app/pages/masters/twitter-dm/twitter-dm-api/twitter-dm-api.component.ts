...
import { masters, regex, countryCode, twitterDmsteps, checknull, checknull1 } from 'src/app/global/json-data';

@Component({
  selector: 'app-twitter-dm-api',
  templateUrl: './twitter-dm-api.component.html',
  styleUrls: ['./twitter-dm-api.component.scss']
})
export class TwitterDmApiComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  constructor(public common: CommonService,
    ...
    private el: ElementRef,) {   Object.assign(this, { masters, regex, countryCode, twitterDmsteps }); }
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      SaveIncomingData: ['', [Validators.required,Validators.pattern(regex.alphabet)]],
      SaveOutgoingData: ['', [Validators.required,Validators.pattern(regex.alphabet)]],
      CreateCustomerAgentSession: ['', [Validators.required,Validators.pattern(regex.alphabet)]],
      DeleteCustomerAgentSession: ['', [Validators.required,Validators.pattern(regex.alphabet)]],
      ValidateCustomerAgentSession: ['', [Validators.required,Validators.pattern(regex.alphabet)]],
    },
    {validator:[checknull('SaveIncomingData'),checknull1('SaveIncomingData'),checknull('SaveOutgoingData'),checknull1('SaveOutgoingData'),checknull('CreateCustomerAgentSession'),checknull1('CreateCustomerAgentSession'),checknull('DeleteCustomerAgentSession'),checknull1('DeleteCustomerAgentSession'),checknull('ValidateCustomerAgentSession'),checknull1('ValidateCustomerAgentSession')]},

    )
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {

      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/twitter-dm/twitter-dm-auth', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
  ...
  back(): void{
    this.router.navigate(['masters/channel-configuration/twitter-dm/view/'+this.path]);
  }
  backspace(): void {
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-api', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-media', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
