...
import { masters, regex, countryCode, twitterDmsteps } from 'src/app/global/json-data';

@Component({
  selector: 'app-twitter-dm-attribute',
  templateUrl: './twitter-dm-attribute.component.html',
  styleUrls: ['./twitter-dm-attribute.component.scss']
})
export class TwitterDmAttributeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/twitter-dm/twitter-dm-database', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            else if(event == 'save') this.router.navigate(['masters/channel-configuration/twitter-dm/view/', this.path]);
          }
          cntr++;
        } else {
          this.loader = false;
