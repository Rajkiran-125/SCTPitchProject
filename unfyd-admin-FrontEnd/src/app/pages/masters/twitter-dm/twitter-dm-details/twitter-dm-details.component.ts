...
@Component({
  selector: 'app-twitter-dm-details',
  templateUrl: './twitter-dm-details.component.html',
  styleUrls: ['./twitter-dm-details.component.scss']
})
export class TwitterDmDetailsComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/twitter-dm/twitter-dm-details', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
    ...
    { validator: [checknull('ChannelSourceName'),checknull1('ChannelSourceName'),checknull('VersionNumber'),checknull1('VersionNumber'),checknull('SendMessageUrl'),checknull1('SendMessageUrl'), checknull('SentimentKey'),checknull1('SentimentKey'),checknull('HandleID'),checknull1('HandleID'), checknull('SentimentUrl'),checknull1('SentimentUrl'), checknull('SentimentKey'),checknull1('SentimentKey'), checknull('ScreenName'),checknull1('ScreenName'),checknull('TweetPostLink'),checknull1('TweetPostLink'),checknull('TweetPostLinkMessage'),checknull1('TweetPostLinkMessage')] },
    )
                  ...
                  this.api.post('index', obj).subscribe((res: any) => {
                    if (res.code == 200) {
                      if (temp.length - 1 == counter) {
                        this.loader = false;
                        if(event == 'save') {this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);  this.isUpdate = 'true';}
                        else if(event == 'next')this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-auth', this.activatedRoute.snapshot.paramMap.get('id'), 'update', ChSrcId]);
        ...
        this.api.post('index', obj).subscribe((res: any) => {
          if (res.code == 200) {
            ...
            if (counter == 0) {
              ...
              if(event == 'save') this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-details', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
              else if(event == 'next')this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-auth', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
