...
@Component({
  selector: 'app-twitter-dm-media',
  templateUrl: './twitter-dm-media.component.html',
  styleUrls: ['./twitter-dm-media.component.scss']
})
export class TwitterDmMediaComponent implements OnInit {
  key: any;
  hide = true;
  form: FormGroup;
  ...
  ngOnInit(): void {
    ...
    {validator:[checknull('MediaDownloadAPI'),checknull1('MediaDownloadAPI'),checknull('PageAccessToken'),checknull1('PageAccessToken'),checknull('StorageLocation'),checknull1('StorageLocation')]}
    ...
    if ((this.uniqueId === 'null') || (this.uniqueId === undefined) || (this.uniqueId === null)) {
      ...
      this.loader = false;
      this.router.navigate(['/masters/channel-configuration/twitter-dm/twitter-dm-api', this.path, 'add'])
    }
    else {
      if ((this.uniqueId !== 'null') || (this.uniqueId !== undefined) || (this.uniqueId !== null)) {
        this.isUpdate = 'true';
      ...
      this.api.post('index', obj).subscribe((res: any) => {
        if (res.code == 200) {
          ...
          if (updateData.length - 1 == cntr) {
            ...
            if(event == 'save') this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-media', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
            else if(event == 'next')this.router.navigate(['masters/channel-configuration/twitter-dm/twitter-dm-database', this.activatedRoute.snapshot.paramMap.get('id'), 'update', this.activatedRoute.snapshot.paramMap.get('uniqueid')]);
