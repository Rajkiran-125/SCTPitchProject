...
@Component({
  selector: 'app-interaction-statistics',
  templateUrl: './interaction-statistics.component.html',
  styleUrls: ['./interaction-statistics.component.scss']
})
export class InteractionStatisticsComponent implements OnInit {
  loader: boolean = false;
  spname = "usp_unfyd_hub_dashboard_graphs";
  innerSpName = "HOURLY_STATS"
  flag = "DAY";
  ...
  changeDate(direction: any) {
    this.common.hubControlEvent('Dashboard','click','','',JSON.stringify(direction),'convchangeDateert');
  ...
  getInformation(spname,innerSpName, flag) {
    ...
    this.api
      ...
      .subscribe((res) => {
        this.loader = false;
        if (res.code == 200) {
          this.trainingData = res.results.data[0].HourStats;
