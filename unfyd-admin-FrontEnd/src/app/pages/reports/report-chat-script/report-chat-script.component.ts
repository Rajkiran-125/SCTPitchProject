...
@Component({
  selector: 'app-report-transcripts',
  templateUrl: './report-chat-script.component.html',
  styleUrls: ['./report-chat-script.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS] },
    { provide: MAT_MOMENT_DATE_ADAPTER_OPTIONS, useValue: { useUtc: true } }
  ]
})
export class ReportChatScriptComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.common.hubControlEvent('TranscriptsReport','click','pageload','pageload','','ngOnInit');

    this.userDetails = this.auth.getUser();
    this.type = this.router.url.substring(1, this.router.url.length);
    if (this.type == "report-transcripts") {
    ...
    this.common.hubControlEvent('TranscriptsReport','click','pageloadend','pageloadend','','ngOnInit');

  }

  loadAgentFilter() {
    this.common.hubControlEvent('TranscriptsReport','click','','','','loadAgentFilter');
    ...
    this.subscriptionPopupModal = this.common.agentFilterDynamic.subscribe((data: any) => {
      ...
      this.AgentRouteTime = data?.Number;
      this.page = 1;
      this.Channel = data?.ChannelId;
      this.ClinetCode = data?.ClinetCode;
      this.CustomerName = data?.CustomerName;
      this.Email = data?.Email;
      this.InteractionType = data?.InteractionType;
      this.IsAuthenticate = data?.IsAuthenticate;
  ...
  loadReportsData() {
    this.common.hubControlEvent('TranscriptsReport','click','','','','loadReportsData');
  ...
  pageChange(currentpage) {
    this.common.hubControlEvent('TranscriptsReport','click','','',JSON.stringify(currentpage),'pageChange');
  ...
  getFilter() {
    this.common.hubControlEvent('TranscriptsReport','click','','','','getFilter');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('TranscriptsReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  searchHawker(currentpage, itemsReportPerPage, date) {
    ...
    this.requestObj = {
      data: {
        spname: "USP_REPORT_SWA_ChatScript",
        parameters: {
          ...
          ClinetCode: this.filter == false ? undefined : this.ClinetCode == null ? undefined : this.ClinetCode,
          ...
          IsAuthenticate: this.filter == false ? undefined : this.IsAuthenticate == null ? undefined : this.IsAuthenticate,
          IsRegistered: this.filter == false ? undefined : this.IsRegistered == null ? undefined : this.IsRegistered,
          Message: this.filter == false ? undefined : this.Message == null ? undefined : this.Message,
          MessageTime: this.filter == false ? undefined : this.MessageTime == null ? undefined : this.MessageTime,
          MessageType: this.filter == false ? undefined : this.MessageType == null ? undefined : this.MessageType,
          PhoneNumber: this.filter == false ? undefined : this.PhoneNumber == null ? undefined : this.PhoneNumber,
    ...
    this.common.hubControlEvent('TranscriptsReport','click','','',JSON.stringify( this.requestObj),'searchHawker');
        ...
        if (this.Export == true) {
          ...
          for (let data of tempRes[0]) {
            var newObj = {
              ...
              "AgentRouteTime": data['AgentRouteTime'],
              ...
              "Customer'S 'Name": data['Customer\'s Name'],
              "Phone Number": data['Phone Number'],
              "Email": data['Email'],
              // "ClinetCode": data['ClinetCode'],
  ...
  htmlDecode(data: any) {
    this.common.hubControlEvent('TranscriptsReport','click','','',JSON.stringify( data),'htmlDecode');
  ...
  reset() {
    this.common.hubControlEvent('TranscriptsReport','click','','','','reset');
