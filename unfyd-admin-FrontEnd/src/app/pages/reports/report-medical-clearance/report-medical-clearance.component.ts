...
@Component({
  selector: 'app-report-medical-clearance',
  templateUrl: './report-medical-clearance.component.html',
  styleUrls: ['./report-medical-clearance.component.scss']
})
export class ReportMedicalClearanceComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.common.hubControlEvent('MedicalClearanceReport','click','pageload','pageload','','ngOnInit');
...
this.userDetails = this.auth.getUser();
    ...
    if(this.type == "medical-clearance-report"){
...
this.getFilter();
this.feildChooser()
this.common.hubControlEvent('MedicalClearanceReport','click','pageloadend','pageloadend','','ngOnInit');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('MedicalClearanceReport','click','','',JSON.stringify(obj),'ngOnInit');
  ...
  getFilter() {
    this.common.hubControlEvent('MedicalClearanceReport','click','','','','getFilter');
  ...
  searchHawker() {
    ...
    this.requestObj = {
      data: {
        spname: 'usp_unfyd_medical_clearance_report',
        parameters: {
          FromDate: a,
          ToDate :b
        }
    ...
    this.common.hubControlEvent('MedicalClearanceReport','click','','',JSON.stringify(this.requestObj),'searchHawker');
    ...
    this.api.post('index', this.requestObj).subscribe(res => {
      ...
      if (res.code == 200) {
        ...
        for (let data of tempRes) {
          var newObj = {
            "Sr No": '',
            "Applicant No.": data['Application Number'],
            "Hawker Name": data['Hawker Name'],
            "Doctor": data['Doctor'],
            "Hospital": data['Hospital'],
            "Medical Test Status": data['Medical Test Status'],
            "Date Of Application": data['Date Of Application'],
            "Date of Medical Test": data['Date of Medical Test'],
            "Closure Date Time": data['Closure Date Time'],
  ...
  reset(){
    this.common.hubControlEvent('MedicalClearanceReport','click','','','','reset');
