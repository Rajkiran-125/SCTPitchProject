...
@Component({
  selector: 'app-report-interaction-dashboard',
  templateUrl: './report-interaction-dashboard.component.html',
  styleUrls: ['./report-interaction-dashboard.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS] },
    { provide: MAT_MOMENT_DATE_ADAPTER_OPTIONS, useValue: { useUtc: true } }
  ]
})
export class ReportInteractionDashboardComponent implements OnInit {
  ...
  ngOnInit() {
    this.common.hubControlEvent('InteractionDashboardReport','click','pageload','pageload','','ngOnInit');

    this.filter = false;
    this.userDetails = this.auth.getUser();
    this.type = this.router.url.substring(1, this.router.url.length);
    if (this.type == "report-interaction-dashboard") {
    ...
    this.common.hubControlEvent('InteractionDashboardReport','click','pageloadend','pageloadend','','ngOnInit');

  }
  loadReportsData() {
    this.common.hubControlEvent('InteractionDashboardReport','click','','','','loadReportsData');
   ...
   loadFilter() {
    this.common.hubControlEvent('InteractionDashboardReport','click','','','','loadFilter');
  ...
  getData(currentpage, itemsPerPage, date) {
    ...
    let requestObj = {
      data: {
        spname: "USP_MASTER_DATA_DASHBOARD",
        parameters: {
    ...
    this.common.hubControlEvent('InteractionDashboardReport','click','','',JSON.stringify(requestObj),'getData');
        ...
        if (this.Export == true) {
          ...
          for (let data of tempRes[0]) {
            var newObj = {
              ...
              "Disconnection Status": data['DisconnectionStatus'],
            }
  ...
  pageChange(currentpage) {
    this.common.hubControlEvent('InteractionDashboardReport','click','','',JSON.stringify(currentpage),'pageChange');
  ...
  getFilter() {
    this.common.hubControlEvent('InteractionDashboardReport','click','','','','getFilter');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('InteractionDashboardReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  htmlDecode(data: any) {
    this.common.hubControlEvent('InteractionDashboardReport','click','','',JSON.stringify(data),'htmlDecode');
