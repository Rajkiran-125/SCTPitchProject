...
@Component({
  selector: 'app-report-pcc',
  templateUrl: './report-pcc.component.html',
  styleUrls: ['./report-pcc.component.scss']
})
export class ReportPccComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.common.hubControlEvent('PCCReport','click','pageload','pageload','','ngOnInit');
...
this.getFilter();
this.feildChooser()
this.common.hubControlEvent('PCCReport','click','pageloadend','pageloadend','','ngOnInit');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('PCCReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  loadAgentFilter() {
    this.common.hubControlEvent('PCCReport','click','','','','loadAgentFilter');
  ...
  searchHawker() {
    ...
    this.common.hubControlEvent('PCCReport','click','','',JSON.stringify(this.requestObj),'searchHawker');
    ...
    this.api.post('index', this.requestObj).subscribe(res => {
      ...
      if (res.code == 200) {
        ...
        for (let data of tempRes) {
          var newObj = {
            "Sr No": '',
            "Applicant No.": data['ApplicationNumber'],
            "Hawker Name": data['HawkerName'],
            "Date Of Application": data['DateOfApplication'],
            "Date Of PCC": data['DateOfPCC'],
            "PCC Reference Number": data['PCCReferenceNumber'],
            "PCC Status": data['PCCStatus'],
            "Police Officer": data['PoliceOfficer'],
  ...
  fromDateChange(event){
    this.common.hubControlEvent('PCCReport','click','','',JSON.stringify(event),'fromDateChange');
  ...
  reset(){
    this.common.hubControlEvent('PCCReport','click','','','','reset');
