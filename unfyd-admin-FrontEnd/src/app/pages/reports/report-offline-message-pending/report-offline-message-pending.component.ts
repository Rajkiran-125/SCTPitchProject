...
@Component({
  selector: 'app-report-offline-message-pending',
  templateUrl: './report-offline-message-pending.component.html',
  styleUrls: ['./report-offline-message-pending.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS] },
    { provide: MAT_MOMENT_DATE_ADAPTER_OPTIONS, useValue: { useUtc: true } }
  ]
})
export class ReportOfflineMessagePendingComponent implements OnInit {
  ...
  ngOnInit() {
    this.common.hubControlEvent('OfflineMessagePendingReport','click','pageload','pageload','','ngOnInit');

    this.filter = false;
    this.userDetails = this.auth.getUser();
    this.type = this.router.url.substring(1, this.router.url.length);
    if (this.type == "report-offline-message-pending") {
    ...
    this.common.hubControlEvent('OfflineMessagePendingReport','click','pageloadend','pageloadend','','ngOnInit');

  }
  loadReportsData() {
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','','','loadReportsData');
  ...
  loadFilter() {
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','','','loadFilter');
  ...
  getData(currentpage, itemsPerPage, date) {
    ...
    let requestObj = {
      data: {
        spname: "UNFYD_TALK_RPT_V1_Pending_OfflineMsgs",
        parameters: {
    ...
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','',JSON.stringify(requestObj),'getData');
  ...
  pageChange(currentpage) {
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','',JSON.stringify(currentpage),'pageChange');
  ...
  getFilter() {
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','','','getFilter');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  htmlDecode(data: any) {
    this.common.hubControlEvent('OfflineMessagePendingReport','click','','',JSON.stringify(data),'htmlDecode');
