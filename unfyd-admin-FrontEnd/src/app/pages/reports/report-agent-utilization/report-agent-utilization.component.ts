...
@Component({
  selector: 'app-report-agent-utilization',
  templateUrl: './report-agent-utilization.component.html',
  styleUrls: ['./report-agent-utilization.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS] },
    { provide: MAT_MOMENT_DATE_ADAPTER_OPTIONS, useValue: { useUtc: true } }
  ]
})
export class ReportAgentUtilizationComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.common.hubControlEvent('AgentUtilizationReport','click','pageload','pageload','','ngOnInit');


    this.userDetails = this.auth.getUser();
    this.proccessId = this.userDetails.Processid;
    this.type = this.router.url.substring(1, this.router.url.length);
    if (this.type == "report-agent-utilization") {
    ...
    this.common.hubControlEvent('AgentUtilizationReport','click','pageloadend','pageloadend','','ngOnInit');

  }

  loadAgentFilter() {
    this.common.hubControlEvent('AgentUtilizationReport','click','','','','loadAgentFilter');
  ...
  loadReportsData() {
    this.common.hubControlEvent('AgentUtilizationReport','click','','','','loadReportsData');
  ...
  getFilter() {
    this.common.hubControlEvent('AgentUtilizationReport','click','','','','getFilter');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('AgentUtilizationReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  searchHawker(currentpage, itemsPerPage, date) {
    ...
    let requestObj = {
      data: {
        spname: "USP_AGENT_UTILISATION_DASHBOARD",
        parameters: {
          ...
          Count: this.filter == false ? undefined : this.Count == null ? undefined : this.Count,
    ...
    this.common.hubControlEvent('AgentUtilizationReport','click','','',JSON.stringify(requestObj),'searchHawker');
  ...
  pageChange(currentpage) {
    this.common.hubControlEvent('AgentUtilizationReport','click','','',JSON.stringify(currentpage),'pageChange');
  ...
  reset() {
    this.common.hubControlEvent('AgentUtilizationReport','click','','','','reset');
