...
@Component({
  selector: 'app-report-grievance-status',
  templateUrl: './report-grievance-status.component.html',
  styleUrls: ['./report-grievance-status.component.scss']
})
export class ReportGrievanceStatusComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.common.hubControlEvent('GrievanceStatusReport','click','pageload','pageload','','ngOnInit');
...
this.userDetails = this.auth.getUser();
    ...
    if(this.type == "grievance-status-report"){
...
this.getFilter();
this.feildChooser()
this.common.hubControlEvent('GrievanceStatusReport','click','pageloadend','pageloadend','','ngOnInit');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('GrievanceStatusReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  getFilter() {
    this.common.hubControlEvent('GrievanceStatusReport','click','','','','getFilter');
  ...
  searchHawker() {
    ...
    this.requestObj = {
      data: {
        spname: 'usp_unfyd_grievance_status_report',
        parameters: {
          FromDate: a,
          ToDate :b
        }
    ...
    this.common.hubControlEvent('GrievanceStatusReport','click','','',JSON.stringify(this.requestObj),'searchHawker');
      ...
      if (res.code == 200) {
        ...
        for (let data of tempRes) {
          var newObj = {
            ...
            "Date": data['Date'],
            "Logged By": data['Logged By'],
            "Logged Date Time": data['Logged Date Time'],
            "Status": data['Status'],
            "Subject": data['Subject'],
            "Unique reference No": data['Unique reference No'],
          }
  ...
  reset(){
    this.common.hubControlEvent('GrievanceStatusReport','click','','','','reset');
