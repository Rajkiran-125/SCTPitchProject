...
@Component({
  selector: 'app-report-funnel-application',
  templateUrl: './report-funnel-application.component.html',
  styleUrls: ['./report-funnel-application.component.scss']
})
export class ReportFunnelApplicationComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.common.hubControlEvent('FunnelApplicationReport','click','pageload','pageload','','ngOnInit');
    ...
    if(this.type == 'funnel-application-report'){
...
this.getFilter();
this.feildChooser()
this.common.hubControlEvent('FunnelApplicationReport','click','pageloadend','pageloadend','','ngOnInit');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('FunnelApplicationReport','click','','',JSON.stringify(obj),'feildChooser');
  ...
  getFilter() {
    this.common.hubControlEvent('FunnelApplicationReport','click','','','','getFilter');
  ...
  searchHawker() {
    ...
    this.requestObj = {
      data: {
        spname: 'usp_unfyd_funnel_application_reg_report',
        parameters: {
          FromDate: a,
          ToDate :b
        }
    ...
    this.common.hubControlEvent('FunnelApplicationReport','click','','',JSON.stringify(this.requestObj),'searchHawker');
      ...
      if (res.code == 200) {
        ...
        for (let data of tempRes) {
          var newObj = {
            ...
            "Train": data['Train'],
            "Route": data['Route'],
            "ER Approved Status": data['ER Approved Status'],
            "Cleared Training Status": data['Cleared Training Status'],
            "Cleared PCC": data['Cleared PCC'],
            "Cleared Medical Test": data['Cleared Medical Test'],
          }
  ...
  reset(){
    this.common.hubControlEvent('FunnelApplicationReport','click','','','','reset');
