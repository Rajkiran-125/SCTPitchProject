...
@Component({
  selector: 'app-report-hsm',
  templateUrl: './report-hsm.component.html',
  styleUrls: ['./report-hsm.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS] },
    { provide: MAT_MOMENT_DATE_ADAPTER_OPTIONS, useValue: { useUtc: true } }
  ]
})
export class ReportHsmComponent implements OnInit {
  ...
  ngOnInit(){
    this.common.hubControlEvent('HSMReport','click','pageload','pageload','','ngOnInit');

    this.filter = false;
    this.userDetails = this.auth.getUser();
    this.type = this.router.url.substring(1, this.router.url.length);
    if (this.type == "report-hsm") {
      this.type = "HSMReport"
    }
    ...
    this.common.hubControlEvent('HSMReport','click','pageloadend','pageloadend','','ngOnInit');

  }
  loadFilter() {
    this.common.hubControlEvent('HSMReport','click','','','','loadFilter');
  ...
  loadReportsData() {
    this.common.hubControlEvent('HSMReport','click','','','','loadReportsData');
  ...
  getData(currentpage, itemsPerPage, date) {
    ...
    let requestObj = {
      data: {
        spname: "UNFYD_ADM_RPT_V1_HSMDETAILS",
        parameters: {
    ...
    this.common.hubControlEvent('HSMReport','click','','',JSON.stringify(requestObj),'getData');
        ...
        if (this.Export == true) {
          ...
          for (let data of tempRes[0]) {
            var newObj = {
              ...
              "Filler 2": data['Filler2'],
              "Filler 3": data['Filler3'],
              "Filler 4": data['Filler4'],
              "Filler 5": data['Filler5'],
              "Filler 6": data['Filler6'],
  ...
  pageChange(currentpage) {
    this.common.hubControlEvent('HSMReport','click','','',JSON.stringify(currentpage),'pageChange');
  ...
  getFilter() {
    this.common.hubControlEvent('HSMReport','click','','','','getFilter');
  ...
  feildChooser() {
    ...
    this.common.hubControlEvent('HSMReport','click','','','','feildChooser');
  ...
  htmlDecode(data: any) {
    this.common.hubControlEvent('HSMReport','click','','',JSON.stringify(data),'htmlDecode');
